...
import {
  ...
  ListPlus, Table2, Trash2, ChevronDown,
} from "lucide-react";
...
export default function BudgetApp() {
  ...
  useEffect(() => {
    if (!loaded) return;
    try {
      localStorage.setItem("budget-data", JSON.stringify({ data, transactions }));
  ...
  const submitTx = (e) => {
    ...
    setTransactions((prev) => [
      { id: Date.now(), ...txForm, amount: Number(txForm.amount) },
      ...prev,
    ]);
...
function TransactionsTab({ txForm, setTxForm, submitTx, transactions, setTransactions, categories, groups }) {
  return (
                ...
                {transactions.map((t) => (
                  <tr key={t.id} style={{ borderTop: `1px solid ${COLORS.line}` }}>
                    ...
                    <td className="py-2 text-right">
                      <button onClick={() => setTransactions((prev) => prev.filter((x) => x.id !== t.id))} style={{ color: COLORS.textDim }}>
                        <Trash2 size={14} />
                      </button>
                    </td>
                  </tr>
